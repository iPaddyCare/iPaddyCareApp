          ...
          <View style={styles.weatherCard}>
            <View style={styles.weatherRow}>
              ...
              <View style={styles.weatherInfo}>
                {loadingWeather ? (
                  <Text style={styles.weatherDescription}>{translate('loadingWeather')}</Text>
                ) : weatherData ? (
                  <>
                    <Text style={styles.weatherLocation}>
                      {translate('common.location')}: {weatherData.locationName || '—'}
                    </Text>
                    <Text style={styles.weatherDescription}>
                      {Number.isFinite(weatherData.temperature) ? weatherData.temperature.toFixed(1) : '—'}
                      {translate('celsius')} • {weatherData.description || ''}
                    </Text>
                  </>
                ) : null}
              </View>
